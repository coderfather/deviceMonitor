Ext.onReady(function() {
	...
	var group3 = Ext.data.Record.create([{
		name : "code",
		type : "string"
	}, {
		name : "name",
	...
	var group3Store = new Ext.data.Store({
			baseParams : {type : 'group3'},
	...
	var cboGroup3 = new Ext.form.ComboBox({
			id : 'cboGroup3',
			name : 'cboGroup3',
			fieldLabel : '分类3',
			selectOnFocus : true,
			hiddenName : 'group3_id',
			valueField : 'code',
			displayField : 'name',
			emptyText : '请选择',
			editable : false,
	...
	var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
							url : '/deviceMonitor/deviceUser/list.do',
							method : "POST"  
						}),
				reader : new Ext.data.JsonReader({
							root : 'list',
	...
	var columnModel = new Ext.grid.ColumnModel([rownum, sm, 
				...
				},
				{
					...
					sortable : false,
					dataIndex : 'group3_id',
					renderer: Ext.util.Format.comboRenderer(cboGroup3)
				},
				{
					header : '配置表版本',
					width : 110,
	...
	function func_form_save() {
		...
		_form.form.findField('user_id').setValue(dmt.user.id);
		var id = _form.form.findField('id').getValue();
		var url = "/deviceMonitor/deviceUser/insert.do";
		var params = {};
		params["action"] = "insert";
		if (id) {
			params["action"] = "update";
			url = "/deviceMonitor/deviceUser/update.do";
		}

		var key = _form.form.findField('device_code').getValue();
		Ext.Ajax.request({
		url : "/deviceMonitor/deviceStation/isExistsDevice.do",
		method : 'POST',
			...
			Ext.MessageBox.confirm('确定删除', '您确定要删除记录吗？', function(btn) {
						if (btn == 'yes') {
							...
							deleteForm.getForm().submit({
										waitTitle : ' ',
										waitMsg : '正在删除，请稍等......',
										url : '/deviceMonitor/deviceUser/delete.do?delIds='
												+ ids,
										params : params,
										failure : function(form, action) {
											var json = action.response.responseText;

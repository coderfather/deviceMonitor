Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
							url : '/deviceMonitor/sysParam/list.do',
							method : "POST"  
						}),
				reader : new Ext.data.JsonReader({
							root : 'list',
	...
	function func_form_save() {
		...
		var url = "/deviceMonitor/sysParam/insert.do";
		var params = {};
		params["action"] = "insert";
		if (id) {
			params["action"] = "update";
			url = "/deviceMonitor/sysParam/update.do";
		}

		var key = _form.form.findField('code').getValue();
		Ext.Ajax.request({
		url : "/deviceMonitor/sysParam/isExists.do",
		method : 'POST',
			...
			Ext.MessageBox.confirm('确定删除', '您确定要删除记录吗？', function(btn) {
						if (btn == 'yes') {
							...
							deleteForm.getForm().submit({
										waitTitle : ' ',
										waitMsg : '正在删除，请稍等......',
										url : '/deviceMonitor/sysParam/delete.do?delIds='
												+ ids,
										params : params,
										failure : function(form, action) {
											var json = action.response.responseText;
